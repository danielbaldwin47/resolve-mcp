...
**Not modules.** `styles/`, `gauntlet/`, `projects/` — agent-owned data, never read by server code; `scripts/` — repo maintenance (`prune_merged.py`, CLAUDE.md step 6) and the workflows' own logic (`review_gate.py`, the `Review: clean @<sha>` verdict `.github/workflows/review-gate.yml` runs, and `review_stamp.py`, which writes that line onto a PR body). Both: `docs/context/repo.md`. ADRs cited above: `docs/adr/`.
