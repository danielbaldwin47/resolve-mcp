...
the shell hook; sub), `test_context_map` (this map covers the tree; pure), `test_prune_merged` (`scripts/prune_merged.py` over an injected `Runner`; pure), `test_review_gate` (`scripts/review_gate.py` over an injected `Runner`; pure), `test_review_stamp` (`scripts/review_stamp.py` over an injected `Runner`; pure), `test_session_start` (the SessionStart hook over an injected `Runner`; pure), `test_gauntlet_imports` (every import in every `gauntlet/` module still resolves; pure). Live tier: `test_live_smoke`, `test_live_analysis`, and
...
**Not modules.** `styles/`, `gauntlet/`, `projects/` — agent-owned data, never read by server code; `scripts/` — repo maintenance (`prune_merged.py`, CLAUDE.md step 6; `_merged.py`, the merged-branch decision it shares with the `.claude/hooks/session-start.py` SessionStart report) and the workflows' own logic (`review_gate.py`, the `Review: clean @<sha>` verdict `.github/workflows/review-gate.yml` runs, and `review_stamp.py`, which writes that line onto a PR body). Both: `docs/context/repo.md`. ADRs cited above: `docs/adr/`.
