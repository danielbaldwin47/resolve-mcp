...
| Beat grid | `analysis/beats` (beat_this, torch) | **CUDA** since #245 — see below | yes — already wired | `device.announce("beat_this")` at inference; `torch` note in the music/structure job results; `+cpu` build and CUDA-build-without-a-card are each a WARNING naming their fix |
| Applause curve | `analysis/applause` (PANNs, torch) | **CUDA** since #245 — same decision | yes — already wired | `device.announce("PANNs")`; same `torch` note, same two WARNINGs |
