...
**Which outlets actually carry it, checked one by one.** The cluster is smaller than it looks and
two of the four usual suspects should be dropped:

- **byteiota** — <https://byteiota.com/davinci-resolve-21-1-mcp-server/> — carries it outright, no
  citation: *"Resolve 21.1's MCP server exposes 88 tools across every major workspace... There are
  also 20 read-only resources covering live system state."* **Wrong.** [press]
- **explainx.ai** — <https://explainx.ai/blog/davinci-resolve-21-1-mcp-server-ai-agents-2026> —
  ...
  callable tools"*, and elsewhere separates Blackmagic's server from community ones correctly.
  **Partly self-correcting.** [press]
- **cutsio** — HTTP 403 to every fetch attempt. **Unverified either way**; do not list it as part of
  the cluster without a browser check. [not-found]
- **kompozy** — fetched successfully and the figure is **not on the page**. It is explicit that AI
  control is Studio-only and calls the MCP workflows "early-stage". **Drop it from the known-wrong
  list.** [press]

Neither CineD nor digitalproduction.com carries the figure at all.

**A second caution the survey produced: advertised tool counts are unreliable in both directions.**
`DigitalWorkflowCompany`'s 88 does check out against its source (counting `@mcp.tool` and
`@mcp.resource` decorators), but `apvlv/davinci-resolve-mcp` advertises 48 and ships **14**, and
`mhadifilms/dvr` advertises "39+" and ships **84**. Any comparison of this project against a
published catalog size should count the decorators, not read the badge — and note that tool count is
the axis this ecosystem competes on (`lordhoell` claims 440+, `CiprianSpiridon` 334, `Airta-Admin`
218, `2sem` 163, `MDizzleZA` 162), which makes the press's choice of 88 as "the" figure arbitrary as
well as misattributed.
...
Network), not a connection count. [not-found]
...
healthy clients: the risk is a stuck holder, not two live ones.
...
destructive operations, operation traces" and "Media analysis, batch CLI, headless edit loop" — and
marks the native server "no" on both. With `OSideMedia`'s README (section b), that is **two authors
independently reaching the same split, in one week, with no audience between them.** It is the
strongest published evidence for coexistence, and it is thin.
...
164-action Resolve bridge with Codex before 21.1". If accurate, that is a practitioner abandoning his
own catalog for the native hatch — the clearest datapoint *against* building a catalog at all. It
could not be traced to a primary post and is quoted at press tier only.
...
- **MCP's own client guidance**,
  <https://modelcontextprotocol.io/docs/develop/clients/client-best-practices>: "Loading every tool
  definition into the model's context window upfront wastes tokens, increases latency, and degrades
  model performance." It names progressive discovery (a `search_tools` meta-tool, full schemas on
  demand, with a "1%–5% of the context window" threshold) and code-mode calling as the two remedies.
  **The native server implements both** — 14 tools, `search_scripting_api` as the discovery tool,
  `run_script` as code mode. Blackmagic built the shape the protocol's own docs recommend, which is
  worth knowing before arguing against it.
...
Three further reports name the *mechanism*, which `hoyt-harness` does not, and between them they
describe ADR 0001's crash exactly: [primary]

- [`samuelgursky#158`](https://github.com/samuelgursky/davinci-resolve-mcp/issues/158) — *"Blackmagic's
  `fusionscript.dll` on Windows is compiled against Python 3.10–3.12 C ABI. When imported under
  Python 3.13, Windows immediately triggers an Access Violation (0xC0000005)."* No traceback; the
  process dies. Corroborated independently by `Tooflex/davinci-resolve-mcp#4`.
- [`samuelgursky#26`](https://github.com/samuelgursky/davinci-resolve-mcp/issues/26) — Resolve's
  loader picks the highest registry-registered Python even when a correct venv is active.
...
So the guard ADR 0001 introduced is *better* evidenced now than when it was written — and a
native-substrate rebuild would be deleting a guard that stays correct for anyone still attaching
externally.

**Third-party handle lifecycles fail in ways the native one is untested for.** [primary]
`samuelgursky#112` — *"Free-edition bridge orphans on Windows: `os.getppid()` never changes, so
`serve()` can never detect Resolve exiting."* `barckley75/resolve-claude-mcp#2` — every call returns
"DaVinci Resolve is not running" while Resolve is open. Both are third-party bugs, not native-server
behaviour; they are listed because they are the failure class the native server has no published
...
Resolve is not the first host app where this happened. Four cases, checked directly: [primary]
...
| Photoshop | `alisaitteke/photoshop-mcp`, 118 tools, ~2026-01 | "Adobe for Creativity" connector, GA 2026-04-28, 50+ tools (<https://blog.adobe.com/en/publish/2026/04/28/adobe-for-creativity-connector>) | **Kept competing on tool count** — pushed 2026-09-15, advertising 118 against Adobe's ~50. |

**In every confirmed case the third party kept going, and none pivoted scope.** The only defensive
action observed anywhere was cosmetic (Blender's rename). No case of dormancy was found — but all
four vendor servers are ≤5 months old, so "nobody has folded yet" may only mean "not yet". Negative
control: no vendor-native Ableton MCP exists as of 2026-09-15 despite `ahujasid/ableton-mcp` running
since 2025-03-19.

**Read carefully, this precedent is weaker than it looks for *this* decision.** Every one of those
projects is a tool catalog competing with another tool catalog, and in three of the four the vendor's
offering was gated behind a paid seat, marked experimental, or limited to one version — a gap to sit
in. Blackmagic's server is none of those: free with Studio, not experimental, updated with the app,
and (per the forum, section a) explicitly open to any MCP client. The precedents say what catalogs
do when a vendor arrives; they say nothing about what a measuring layer should do, because none of
these projects was one.
...
statement exists [not-found]. The vendor precedents above all show third parties surviving — but
every one had a gap to sit in (a paid seat, an experimental plugin, a version floor) that Blackmagic
does not leave.
...
[on-box], and the shape is the one MCP's own client guidance recommends (progressive discovery plus
code mode). It has a named practitioner precedent: Brent Schooley, reported to have built a
164-action Resolve bridge with Codex before 21.1, is quoted abandoning it — *"It's built in and maps
everything in Resolve and will be updated with each release of Resolve. I'll take that over an
external dependency every day of the week."* [press, untraced to a primary post]. "Updated with each
release" is the real argument: Blackmagic's stubs track its own API for free; this repo's wrappers
do not.
...
one 40-call edit, three quarters of it spent discovering the API and writing Python [n=1]. The
concept — *the server measures; Claude decides* — is the measuring half, and the measuring half is
exactly what has no native path.
...
evidence for where the native server's boundary actually falls (section b). A third document reaches
the same line from outside both projects: `comfyui-cinema-pipeline`'s working rule assigns "media
analysis, batch CLI, headless edit loop" and destructive-operation guardrails to the community server
and marks the native server "no" on both (section c).
...
- **The silent majority.** Most of the ~31 third-party repos surveyed have said nothing about 21.1.
  Whether that becomes engagement or dormancy is the most informative thing a re-survey in a month
  would show — and the vendor precedents (Blender, Figma, Unreal, Photoshop) are all ≤5 months old,
  so their "nobody folded" outcome is not yet a settled one either.
