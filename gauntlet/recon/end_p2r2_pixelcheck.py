"""Pixel check + tail proof on the round-2 ending render. READ-ONLY.
...
RENDER = HERE.parents[1] / "gauntlet" / "renders" / "taurus_ending_p2r2.mp4"
OUT = HERE / "end_p2r2_pixelcheck.json"
...
def main() -> None:
    ...
    luma = series(
        ...
        ["-vf", "fps=8,signalstats,metadata=print:key=lavfi.signalstats.YAVG:"
                "file=end_p2r2_luma.txt"],
        "lavfi.signalstats.YAVG=",
        HERE / "end_p2r2_luma.txt",
    )
    rms = series(
        RENDER,
        ["-af", "astats=metadata=1:reset=24,ametadata=print:"
                "key=lavfi.astats.Overall.RMS_level:file=end_p2r2_rms.txt"],
        "lavfi.astats.Overall.RMS_level=",
        HERE / "end_p2r2_rms.txt",
    )
    ...
    # The band has to be on screen, at full brightness, for the final punch. Three frames:
    # the last note (83.40), one frame before the dissolve begins (83.88), and mid-dissolve.
    grabs = HERE / "end_p2r2_grabs"
    grabs.mkdir(exist_ok=True)
    rep["grabs"] = []
    for at, tag in ((83.40, "last_note"), (83.88, "pre_dissolve"), (86.50, "mid_dissolve")):
