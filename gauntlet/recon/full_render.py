"""Put the whole-song timeline on 1080p custom settings, render it, restore Zinc SYNC.

G13: a build inherits the project's 4K default, so the resolution is set on the timeline BEFORE
the render rather than after it. The closing check also reports any tail-staging timeline left in
the project -- the hardened round trip is supposed to leave none.
"""
