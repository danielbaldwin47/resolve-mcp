"""Validate + build the round-2 piece-3 cut, then run the correlate self-review.

Same shape as mid_p3_build.py, retargeted at taurus-people-mid-p3r2.cut.json, and it prints
the shot_rhythm block as well as the offsets: round 1 lost on rhythm, so reads_metronomic is
a self-review gate here alongside the 17-41 ms band.
"""
...
CUT = HERE.parents[1] / "projects" / "mcp-tests-zinc" / "taurus-people-mid-p3r2.cut.json"
OUT = HERE / "mid_p3r2_build.json"
...
def main() -> None:
    ...
    r = res.get("result") or {}
    for key in ("transient_offsets", "shot_seconds", "shot_rhythm", "clips", "roles",
                "visible", "gated", "stranded", "openings", "outside_grid"):
        print(f"{key}: {json.dumps(r.get(key), default=str)[:1200]}", flush=True)
