"""Pixel check on the round-2 render: ffprobe shape, scene-detect count vs item count.

Scene detect at 0.10 over the render, matched against the 13 planned cut times (the timeline
is 14 butt-joined items, so a clean render shows 13 scene changes). A cut the detector misses
is a cut between two shots of the same picture, which this cut has none of by construction.
"""
...
OUT = HERE / "mid_p3r2_pixelcheck.json"
FPS = 24000.0 / 1001.0
...
def main() -> None:
    ...
    planned = [round(c["cut_frame_rel"] / FPS, 3) for c in PLAN["cuts"]]
    det = scenes(0.10)
    matched, unmatched = [], []
    for c in planned:
    ...
    print(json.dumps({k: rep[k] for k in
                      ("duration_s", "video", "audio_streams", "timeline_items", "planned_cuts",
                       "matched", "unmatched_planned", "extra_detections")}, indent=1), flush=True)
