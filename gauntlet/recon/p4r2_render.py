"""Drop the superseded R2 builds, put the kept one on 1080p, render it, restore Zinc SYNC.

G13: a build inherits the project's 4K default, so the resolution is set on the timeline BEFORE
the render rather than after it. The closing check reports any tail-staging timeline left in the
project -- the hardened round trip is supposed to leave none -- and confirms Zinc SYNC is current.
"""
...
NAME = "taurus_full_p4r2"
SUPERSEDED = ("Taurus People Full P4 R2 v1", "Taurus People Full P4 R2 v2")
...
for i in range(project.GetTimelineCount(), 0, -1):
    ...
    if tl.GetName() in {SUPERSEDED!r}:
        gone.append(tl.GetName())
        pool.DeleteTimelines([tl])
result = {{'deleted': gone,
           'left': [project.GetTimelineByIndex(i).GetName()
...
result = {{'before': before, 'ok_w': ok_w, 'ok_h': ok_h,
           ...
           'video_tracks': found.GetTrackCount('video')}}
"""
...
def main() -> None:
    ...
    print("drop:", json.dumps(escape_hatch.run_python(code=DROP), default=str)[:900], flush=True)
    print("hd:", json.dumps(escape_hatch.run_python(code=SET_HD), default=str)[:800], flush=True)
    ...
    while True:
        ...
        if step != last:
            print(f"  [{round(time.time() - began)}s]", step, flush=True)
            last = step
