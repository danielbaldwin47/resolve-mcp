"""Validate + build the Taurus opening cut, then run the mandatory self-review.

One long-lived process: jobs are in-process daemon threads, so the correlate job
is polled here rather than across tool calls.
"""
...
def main() -> None:
    ...
    report["build"] = built
    name = (built.get("timeline") or {}).get("name") if isinstance(built.get("timeline"), dict) else built.get("timeline")
    print("built:", name, flush=True)
    print("build keys:", list(built), flush=True)
    ...
    print(json.dumps(res.get("result"), indent=1, default=str)[:6000], flush=True)
