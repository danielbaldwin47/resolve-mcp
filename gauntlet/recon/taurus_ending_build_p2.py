"""Validate + build the piece-2 Taurus ENDING cut, then run the correlate self-review.

One long-lived process: jobs are in-process daemon threads, so the correlate job is
polled here rather than across tool calls. Same shape as taurus_build_r3.py.
"""
...
OUT = HERE / "taurus_ending_build_p2.json"
...
def main() -> None:
    ...
    print("SUMMARY:", json.dumps(keep, indent=1, default=str)[:3000], flush=True)
