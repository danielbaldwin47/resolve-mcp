"""Validate + build the piece-2 ROUND-2 Taurus ENDING cut, then run the correlate self-review.

One long-lived process: jobs are in-process daemon threads, so the correlate job is polled
here rather than across tool calls. Same shape as taurus_ending_build_p2.py, plus the tail
block of the build report printed back, because the tail is the thing this round exists to
get right and it is invisible in every other field.
"""
...
OUT = HERE / "taurus_ending_build_p2r2.json"
