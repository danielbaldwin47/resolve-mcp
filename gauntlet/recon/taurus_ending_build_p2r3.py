"""Validate + build the piece-2 ROUND-3 Taurus ENDING cut, then run the correlate self-review.

One long-lived process: jobs are in-process daemon threads, so the correlate job is polled
here rather than across tool calls. Same shape as taurus_ending_build_p2r2.py. The tail
block is printed back in full because the hardened round trip (b743b7e) now re-reads the
LANDED timeline -- placements and both fades -- so `confirmed` is a statement about the
timeline that was delivered rather than about the staging one that was thrown away.
"""
...
OUT = HERE / "taurus_ending_build_p2r3.json"
...
def main() -> None:
    ...
    print("PLACED:", json.dumps(built.get("placed"), default=str), flush=True)
    ...
    print("SUMMARY:", json.dumps(keep, indent=1, default=str)[:4000], flush=True)
