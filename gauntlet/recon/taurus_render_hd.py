"""Put the built opening timeline on 1080p custom settings, re-render, restore Zinc SYNC."""
...
def main() -> None:
    ...
    print("hd:", json.dumps(escape_hatch.run_python(code=SET_HD), default=str)[:600], flush=True)
    ...
    while True:
        ...
        if j.get("state") in ("completed", "failed"):
            print(json.dumps(j.get("result") or j, indent=1, default=str)[:1200], flush=True)
            break
        time.sleep(4.0)
