...
asked it. 7 rather than 6 because every record now carries ``delta`` and ``jump_cut``, which
are ``None`` on a call that named no cut-delta catalog — and a cached reading-6 file would
hand back a report with no such columns at all.
"""
...
def correlate_timeline(
    ...
    watched = [
        {"reading": READING},
        cut.fingerprint,
        *_fingerprints(beats, audio, tunes, solos, deltas),
    ]
